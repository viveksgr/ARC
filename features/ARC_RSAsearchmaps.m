...
modelname = 'searchl_chem';
savepath = fullfile('C:\Work','\ARC\ARC\RSA',modelname);
mkdir(savepath)
...
singletrialer = true;
percepter = false; % Use perceptual rather than chemical ratings
nodor = 160;
mapper  = true;
...
for s = [1 2 3] % Subject
    ...
    for ii = 1:length(anat_names)
        ...
        if DNNner
            Pmat_val = squeeze(h2(s,group_vec,:));
            Pmat_sal = squeeze(h2_sal(s,group_vec,:));
        else
        ...
        for cnt2 = 1:nvox
            ...
            if (size(S_omat_vals_r,1)>1)
                ...
                if sum(p_values_val<0.05)>=1
                    rsa_vec_val(cnt2,1) = correlateDistanceMatrix2(M_anat, Pmat_val,[],false);
                end
                if sum(p_values_sal<0.05)>=1
                    rsa_vec_sal(cnt2,1) = correlateDistanceMatrix2(M_anat, Pmat_sal,[],false);
                end
            end
        end
        ...
        if mapper
            ...
            if percepter
                write_reshaped_nifty(rsa_vec_val, savepath, false, fullfile(statpath,'anat_gw.nii'), sprintf('P_ARC%02d_val_RSAx1%s',s,anat_names{ii}));
                write_reshaped_nifty(rsa_vec_sal, savepath, false, fullfile(statpath,'anat_gw.nii'), sprintf('P_ARC%02d_sal_RSAx1%s',s,anat_names{ii}));
            else
                write_reshaped_nifty(rsa_vec_val, savepath, false, fullfile(statpath,'anat_gw.nii'), sprintf('C_ARC%02d_val_RSAx1%s',s,anat_names{ii}));
                write_reshaped_nifty(rsa_vec_sal, savepath, false, fullfile(statpath,'anat_gw.nii'), sprintf('C_ARC%02d_sal_RSAx1%s',s,anat_names{ii}));
            end
        end
    end
    %}
...
yline(r2t(0.05,nchoosek(ntrials,2)));
% yline(5);
