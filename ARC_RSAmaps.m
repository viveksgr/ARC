...
%     M_val = -(behav_ratings_.*behav_ratings_');
    ...
    %     [~,argsort2] = sort(abs(behav_ratings_));
    if ~val_sep
        ...
        behav_ratings_old = behav_ratings_;
        neg_id = behav_ratings_<md;
        pos_id = behav_ratings_>=md;
        behav_neg  = zscore(behav_ratings_(neg_id));
        behav_pos =  zscore(behav_ratings_(pos_id));     

        % behav_neg = normalize(behav_ratings_(neg_id),'medianiqr');
        % behav_pos = normalize(behav_ratings_(pos_id),'medianiqr');

        % behav_neg = behav_ratings_(neg_id);
        % behav_pos = behav_ratings_(pos_id);
        % behav_ratings_(neg_id)=behav_neg;
        % behav_ratings_(pos_id)=behav_pos;
        
        M_val = abs(behav_ratings_-behav_ratings_');
        M_valpos = abs(behav_pos-behav_pos');
        M_valneg = abs(behav_neg-behav_neg');
    end
...
%             else
            ...
            S1_pos = S1_omat_vals(:,pos_id);
            S1_neg = S1_omat_vals(:,neg_id);
            S1_pos = zscore(S1_pos,[],2);
            S1_neg = zscore(S1_neg,[],2);

            M_anatpos = 1-corrcoef(S1_pos);
            M_anatneg = 1-corrcoef(S1_neg);

            utl_mask_pos = logical(triu(ones(sum(pos_id)),1)); % All possible odors
            utl_mask_neg = logical(triu(ones(sum(neg_id)),1)); % All possible odors

            if partial_corr

                unitypos = unity(pos_id,pos_id);
                unityneg = unity(neg_id,neg_id);
                M_anatpos = ARC_applyFunctionToUpperTriangle(M_anatpos, unitypos(utl_mask_pos));
                M_anatneg = ARC_applyFunctionToUpperTriangle(M_anatneg, unityneg(utl_mask_neg));
            end
            ...
            % Valence - pos
            temp = corrcoef(M_anatpos(utl_mask_pos),M_valpos(utl_mask_pos));
            if spearmanner
                temp = corr(M_anatpos(utl_mask_pos),M_valpos(utl_mask_pos),'type','spearman');
                temp(2) = temp;
            end  
            S_mat(ii,1) = temp(2);
            [~,S_mat2(ii,1)] = r2p(double(temp(2)),sum(utl_mask_pos(:)));

             % Valence - neg
            temp = corrcoef(M_anatneg(utl_mask_neg),M_valneg(utl_mask_neg));
            if spearmanner
                temp = corr(M_anatneg(utl_mask_neg),M_valneg(utl_mask_neg),'type','spearman');
                temp(2) = temp;
            end
            S_mat(ii,2) = temp(2);
            [~,S_mat2(ii,2)] = r2p(double(temp(2)),sum(utl_mask_neg(:)));
        end     
    end
    rsa_P1(s,:,:) = S_mat;
    rsa_P2(s,:,:) = S_mat2; 
...
yline(r2t(0.05,sum(utl_mask_pos(:))),'--');
yline(r2t(0.05,sum(utl_mask_neg(:))),'*');
if val_sep
    legend({'Val+','Val-'})
else
    legend({'Valence','Salience'})
end
savefig(fullfile(savepath,'ARC_RSA_tsc'))
print(fullfile(savepath,'ARC_RSA_tsc'),'-dpng')
