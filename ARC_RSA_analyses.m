    ...
    if v_id==0
        ...
        for ii = 1:length(anat_names)
            ...
            if sz_ctrl
                ...
                nROI = 254; nvox = size(modelmd_binned,1);
                md2 = zeros(nROI,binz,nshuff);
                md2_shuff = zeros(nshuff,nROI,binz);
                for zzt = 1: nROI
                    perm_idx = datasample(1:nvox, nROI);
                    md2(:,:,zzt) = modelmd_binned_mat(perm_idx,:,zzt);
                    md2_shuff(zzt,:,:) = modelmd_binned_shuff(zzt,perm_idx,:);
                end
                modelmd_binned_mat = md2;
                modelmd_binned_shuff = md2_shuff;
            end
