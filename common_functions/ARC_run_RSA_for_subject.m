...
for zz = 1:cfg.nShuffle % shuffle
    ...
    if cfg.valenceSplit_nov
        m_neg = and(maskMat,logical(blkdiag(ones(cfg.binzPart1),zeros(cfg.binzPart1-1))));
        m_pos = and(maskMat,logical(blkdiag(zeros(cfg.binzPart1-1),ones(cfg.binzPart1))));
        [b1,~] = ARC_multicomputeWeights_tsc([val1(m_neg)],modelmd_corrcoef(m_neg));
        [b2,~] = ARC_multicomputeWeights_tsc([val1(m_pos)],modelmd_corrcoef(m_pos)); % Positive domain
        wt_dist(zz,:) = [0 b2(2) b1(2)];
    else
...
for zz = 1:cfg.nShuffle % shuffle
    ...
    if cfg.valenceSplit_nov
        if cfg.ignore_neut; d1 = 1; d2 = 0; else; d1 = 0; d2 = 1; end
        m_neg = and(maskMat,logical(blkdiag(ones(cfg.binzPart1-d1),zeros(cfg.binzPart1-d2))));
        m_pos = and(maskMat,logical(blkdiag(zeros(cfg.binzPart1-d2),ones(cfg.binzPart1-d1))));
        [b1,~] = ARC_multicomputeWeights_tsc([val1(m_neg)],modelmd_corrcoef(m_neg));
        [b2,~] = ARC_multicomputeWeights_tsc([val1(m_pos)],modelmd_corrcoef(m_pos)); % Positive domain
        wt_dist(zz,:) = [0 b2(2) b1(2)];
    else
...
if cfg.plsregress
    ...
    v11 = logical([0 0 0 0 0 1 1]);
    v22 = logical([1 1 0 0 0 0 0]);
