...
%% Group plots across bars
dirs = dir(fullfile(pwd,'Basic*'));
dirs_ = {}; for ii = 1:4; dirs_{ii} = fullfile(dirs(ii).folder, dirs(ii).name); end
var_set = variable_extract(dirs_,'ARC_decoding.mat','rsa_P1',false);
var_set = cat(4,var_set{:});

% var_set = variable_extract(dirs_,'ARC_decoding.mat','rsa_P1',false);
% var_set = cat(4,var_set{:});

[p_mat, out] = pvals_across_params(var_set, ...
    'FisherZ', true, ...
    'Method',  'rm', ...
    'ParamLabels', {'3','5','7','9'});


plot_varset_roi_lines(var_set, ...
    'RoiLabels', anat_names , ...
    'CondLabels', {'Valence','Salience'}, ...
    'ParamLabels', {'3','5','7','9'}, ...
    'YLabel', 'Accuracy (r)', ...
    'Title', 'Condition profiles across bins');
